...
sparkle_builder.buildAccordionPanel = function () {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: "SELECT * WHERE { ?sub ?pred ?obj . } LIMIT 20",
        //dataType: 'jsonp',
        crossdomain: true,
    ...
    function parseOntologyURI(str){
        var splitstr = str.split("/")
        return splitstr[splitstr.length-1]
    }
    function setupquerybuilder(data){
        console.log("CONFIRMING THIS IS RUNNING")
        var results = data['results']['bindings']
        //var nouns = ['Japanese', 'Airplane', 'Freighter', 'Fighter', 'Length', 'Air Battery', 'Shenyang J-11', 'Width', "Jet"]
        var nouns = []
        for(var i = 0 ; i < results.length; i++){
            nouns.push(parseOntologyURI(results[i]['sub']['value']))
